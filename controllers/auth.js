  ...
  try {
    ...
    if (name.length > 30) {
      return res.status(400).json({
        success: false,
        msg: "Your fullname must be 30 characters or less, including spaces.",
      });
    }
    ...
    if (!/^\d+$/.test(accountId)) {
      return res.status(400).json({
        success: false,
        msg: "Your account ID should contain numbers only.",
      });
    }
    if (accountId.length !== 10) {
      return res.status(400).json({
        success: false,
        msg: "Your account ID must be exactly 10 digits long.",
      });
    }
    ...
    if (!/^\d+$/.test(password)) {
      return res.status(400).json({
        success: false,
        msg: "Your password should contain numbers only.",
      });
    }
    if (password.length !== 4) {
      return res.status(400).json({
        success: false,
        msg: "Your password must be exactly 4 digits long.",
      });
    }
    ...
    if (checkUser) {
      return res.status(401).json({
        success: false,
        msg: "This account ID is already in use. Please use a different account ID.",
      });
    }
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({
        success: false,
        msg: "User not found. Please check your account ID.",
      });
    }
